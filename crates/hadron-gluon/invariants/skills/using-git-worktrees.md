...
1. **Location Priority:**
   ...
   - Existing `.worktrees/` directory (or `worktrees/`).
   - Default: `.worktrees/` at repository root.

2. **Gitignore Safety Verification:**
   ```bash
   git check-ignore -q .worktrees 2>/dev/null || git check-ignore -q worktrees 2>/dev/null
   ```
   *If NOT ignored:* Add location to `.gitignore` and commit before creation.
...
3. **Creation:**
   ...
   cd "$path"
   ```
   *Sandbox Fallback:* If blocked by sandbox permissions, report error and work in current directory.
