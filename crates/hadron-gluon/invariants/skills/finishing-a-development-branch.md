...
```
*Note: Do NOT remove worktree (user requires it for PR feedback).*
...
In Hadron, worktrees live under `.hadron/trees/<quark>`. Hadron Gluon harness owns and recycles all worktrees. **Quarks must NEVER execute `git worktree remove` or delete worktree directories directly.**
...
- **NEVER** run `git worktree remove` inside `.hadron/trees/<quark>`.
