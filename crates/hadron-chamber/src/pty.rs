...
impl PtyTerminal {
    ...
    pub fn new(cwd: &Path, cols: usize, rows: usize) -> Result<Self, String> {
        ...
        // The slave handle MUST be dropped after spawning the child. On Windows
        // ConPTY, holding it open prevents the master reader from receiving the
        // shell's output — the terminal renders as a black screen with cursor.
        drop(pair.slave);
