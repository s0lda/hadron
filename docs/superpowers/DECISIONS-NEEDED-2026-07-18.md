...
Autonomous run while you're out. I build the **safe + verifiable + reversible** parts and **stop at anything that's a security/product _policy_ call** — those are below. Nothing here is blocking the code that's shipping; these gate the parts I deliberately won't guess.
...
- ✅ **Personas** (merged): `~/.hadron/agents/` + repo `.hadron/agents/` load; `@persona-name` routes via its `preferred_role` (reuses role routing), hermetic + whole-branch-reviewed. Persona _instructions_ injection (a persona's body → the seat's prompt) is deferred — routing is the core; when you pick it up, `adapter/prompt.rs` should also start telling quarks `@persona-name` is a valid address.
...
Think we could copy behaviour of mode for this, although different providers by have different or no efforts, maybe we should change it to just default and if provider gives option, then we show them?

## DECISIONS THAT GATE THE §2 ACTIVATION WORK (I will NOT guess these)

The §2 No-Human-Mode gate is built + reviewed but **inert** (toggle off). Before it can be _activated_, these are yours:
...
2. **Where do the allow/deny lists live, and in what form?** You said exact-match **and** prefix/glob (good). But storage is open: `team.json`? a new `~/.hadron/permissions.json` / `.hadron/permissions.json`? field `ModeSet`-style events? Pick one and I'll build the loader + the `decide()` wiring (the matcher already supports exact + `*`-glob).
   Perhaps they could live in `team.json` each model could have commands: [allowed: {}, not-allowed:{}]
...
- **Custom script tools (`.py`/`.rs` run in the tree):** you said "add a sandboxed-or-not toggle." That's the _shape_, but I won't build the execution path unattended
  proceed with it, its kind of basic thing in LLM CLI's
...
- **UI pixel/size:** tags are now `xsmall` and in-row — if still too big/small or the grey "Default" isn't grey enough in your theme, tell me the direction. - they are in the column, its okay, perhaps add extra 100px width to left sidebar?
- **SDK adapter:** native Rust Gemini client vs. just honestly renaming `acp-agy` (no native Rust SDK exists unless we build it). - just rename it, we wont build native one if its only support Gemini API keys, no support for users with AI plans.
- **Branch consolidation / push:** everything's on `main` locally, not pushed. Say if you want `main` pushed. - once you are done with all.
