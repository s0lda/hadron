...
- **Leading doc-comments (`///`) and attributes (`#[...]`) are NOT in a block's byte span.** Verified against tree-sitter-rust: `///` parses as a sibling `line_comment` and `#[inline]` as a sibling `attribute_item`, both *outside* the `function_item`/`struct_item` node. Consequences for v1: (a) the hash covers only the item body, so editing the doc text does **not** invalidate the block's hash; (b) an `apply_edit` replacement leaves the leading docs/attributes untouched in place (the agent's `new_text` should therefore be the item body only). When the engine wires this to real multi-agent edits, decide deliberately whether to extend the block span to include leading trivia (so doc edits are hash-tracked) or keep the body-only contract; either is fine but must be chosen, not left implicit.
