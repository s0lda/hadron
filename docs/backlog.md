...
| 29 | Skills: declared procedures + engine-checked separation of duties | 🟡 | `77247d5` — **was NOT blocked on #27**, the injection/routing half needed no isolation. `writing-plans` / `executing-plans` / `reviewing-work` compiled into `invariants/skills/`, one selected per turn by the engine from the task text; the engine names the peers who can actually take the next step (roster − self − disabled − depleted) and, reading `author:` off the plan on disk, refuses to let a quark verify a plan it wrote. **`proven: false`** — no live turn has been dispatched into it yet; asserted through `prompt::build`. The remaining half — engine-checked **exit criteria** (a turn that claims done vs. what the repo says) — is still open and *does* need #27 |
